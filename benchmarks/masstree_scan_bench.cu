...
bench_rates bench_masstree_range_query(thrust::device_vector<key_slice_type>& d_keys,
  ...
  cudaStream_t successor_stream{0};
  float average_insertion_seconds(0.0f);
  float average_successor_seconds(0.0f);
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    gpu_timer successor_timer(successor_stream);
    successor_timer.start_timer();
    tree.scan(d_query_keys.data().get(), d_query_lengths.data().get(),
              max_key_length, max_counts_per_query, num_keys,
              nullptr, nullptr, nullptr, d_query_results.data().get(), nullptr, nullptr, successor_stream);
    ...
    std::cout << successor_elapsed << std::setw(6) << '\n';
  }
...
int main(int argc, char** argv) {
  ...
  uint32_t max_counts_per_query = get_arg_value<uint32_t>(arguments, "max-counts-per-query").value_or(1u);
  ...
  auto d_results   = thrust::device_vector<value_type>(num_keys * max_counts_per_query, invalid_value);
  ...
  bench_masstree_range_query<masstree_slab_reclaim_type, false>(
  ...
  bench_masstree_range_query<masstree_slab_reclaim_type, true>(
    d_keys, d_lengths, d_values, d_find_keys, d_find_lengths, d_results,
    num_keys, max_key_length, max_counts_per_query, num_experiments
  );
}
